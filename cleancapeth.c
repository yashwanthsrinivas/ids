...
int main(int argc , char ** argv ){
  ...
  while(1)
    {
      struct option optionlist[10]=  {
	{ "filter", required_argument , 0, 'f' } ,
	{"verbose",no_argument, 0, 'v'  } ,
        { "interface",required_argument , 0 ,'i'} ,
	{"noofpackets",required_argument,0,'c' } ,
	{0,0,0,0 } };



      returnvalue=getopt_long(argc , argv , "f:i:c:v" , optionlist  , &optionindex );
	...
	break;
      ...
      case 'f' :
	printf("filter is %s ", optarg );
	strcpy(filter,optarg );                                                                                                                      
	break;

      case 'i' :
	printf("interface is %s ", optarg );
  ...
  char *er;
  pcap_t * ihandle=pcap_open_live(interface,25,0,10,er);
